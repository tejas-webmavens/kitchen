...
class Core_BP_Raven_Client
{
    ...
    public function __construct($options_or_dsn = null, $options = array())
    {
        ...
        $this->logger = Core_BP_Raven_Util::get($options, 'logger', 'php');
        $this->server = Core_BP_Raven_Util::get($options, 'server');
        $this->secret_key = Core_BP_Raven_Util::get($options, 'secret_key');
        $this->public_key = Core_BP_Raven_Util::get($options, 'public_key');
        $this->project = Core_BP_Raven_Util::get($options, 'project', 1);
        $this->auto_log_stacks = (bool) Core_BP_Raven_Util::get($options, 'auto_log_stacks', false);
        $this->name = Core_BP_Raven_Util::get($options, 'name', Core_BP_Raven_Compat::gethostname());
        $this->site = Core_BP_Raven_Util::get($options, 'site', Core_BP_Raven_Client::_server_variable('SERVER_NAME'));
        $this->tags = Core_BP_Raven_Util::get($options, 'tags', array());
        $this->release = Core_BP_Raven_Util::get($options, 'release', null);
        $this->environment = Core_BP_Raven_Util::get($options, 'environment', null);
        $this->sample_rate = Core_BP_Raven_Util::get($options, 'sample_rate', 1);
        $this->trace = (bool) Core_BP_Raven_Util::get($options, 'trace', true);
        $this->timeout = Core_BP_Raven_Util::get($options, 'timeout', 2);
        $this->message_limit = Core_BP_Raven_Util::get($options, 'message_limit', self::MESSAGE_LIMIT);
        $this->exclude = Core_BP_Raven_Util::get($options, 'exclude', array());
        $this->excluded_exceptions = Core_BP_Raven_Util::get($options, 'excluded_exceptions', array());
        $this->severity_map = null;
        $this->http_proxy = Core_BP_Raven_Util::get($options, 'http_proxy');
        $this->extra_data = Core_BP_Raven_Util::get($options, 'extra', array());
        $this->send_callback = Core_BP_Raven_Util::get($options, 'send_callback', null);
        $this->curl_method = Core_BP_Raven_Util::get($options, 'curl_method', 'sync');
        $this->curl_path = Core_BP_Raven_Util::get($options, 'curl_path', 'curl');
        $this->curl_ipv4 = Core_BP_Raven_Util::get($options, 'curl_ipv4', true);
        $this->ca_cert = Core_BP_Raven_Util::get($options, 'ca_cert', Core_BP_Raven_Client::get_default_ca_cert());
        $this->verify_ssl = Core_BP_Raven_Util::get($options, 'verify_ssl', true);
        $this->curl_ssl_version = Core_BP_Raven_Util::get($options, 'curl_ssl_version');
        $this->trust_x_forwarded_proto = Core_BP_Raven_Util::get($options, 'trust_x_forwarded_proto');
        $this->transport = Core_BP_Raven_Util::get($options, 'transport', null);
        $this->mb_detect_order = Core_BP_Raven_Util::get($options, 'mb_detect_order', null);
        $this->error_types = Core_BP_Raven_Util::get($options, 'error_types', null);

        // app path is used to determine if code is part of your application
        $this->setAppPath(Core_BP_Raven_Util::get($options, 'app_path', null));
        $this->setExcludedAppPaths(Core_BP_Raven_Util::get($options, 'excluded_app_paths', null));
        // a list of prefixes used to coerce absolute paths into relative
        $this->setPrefixes(Core_BP_Raven_Util::get($options, 'prefixes', Core_BP_Raven_Client::getDefaultPrefixes()));
        $this->processors = Core_BP_Raven_Client::setProcessorsFromOptions($options);
        ...
        $this->context = new Core_BP_Raven_Context();
        $this->breadcrumbs = new Core_BP_Raven_Breadcrumbs();
        $this->_shutdown_function_has_been_set = false;
        $this->useCompression = function_exists('gzcompress');
        
        $this->sdk = Core_BP_Raven_Util::get($options, 'sdk', array(
            'name' => 'sentry-zend-php',
            'version' => self::VERSION,
        ));
        $this->serializer = new Core_BP_Raven_Serializer($this->mb_detect_order);
        $this->reprSerializer = new Core_BP_Raven_ReprSerializer($this->mb_detect_order);

        if ($this->curl_method == 'async') {
            $this->_curl_handler = new Core_BP_Raven_CurlHandler($this->get_curl_options());
        }

        $this->transaction = new Core_BP_Raven_TransactionStack();
        ...
        if (Core_BP_Raven_Util::get($options, 'install_default_breadcrumb_handlers', true)) {
            $this->registerDefaultBreadcrumbHandlers();
        }

        if (Core_BP_Raven_Util::get($options, 'install_shutdown_handler', true)) {
    ...
    public function install()
    {
        if ($this->error_handler) {
            throw new Core_BP_Raven_Exception(sprintf('%s->install() must only be called once', get_class($this)));
        }
    ...
    {
        return array(
            'Core_BP_Raven_Processor_SanitizeDataProcessor',
        );
    }
    ...
    {
        $processors = array();
        foreach (Core_BP_Raven_Util::get($options, 'processors', Core_BP_Raven_Client::getDefaultProcessors()) as $processor) {
            /**
    ...
    public function captureException($exception, $data = null, $logger = null, $vars = null)
    {
        ...
        do {
            ...
            $exc_data['stacktrace'] = array(
                'frames' => Core_BP_Raven_Stacktrace::get_stack_info(
    ...
    {
        $handler = new Core_BP_Raven_Breadcrumbs_ErrorHandler($this);
        $handler->install();
    }
    ...
    {
        $message = Core_BP_Raven_Compat::json_encode($data);
        if ($message === false) {
    ...
    public function translateSeverity($severity)
    {
        ...
        switch ($severity) {
            case E_ERROR:              return Core_BP_Raven_Client::ERROR;
            case E_WARNING:            return Core_BP_Raven_Client::WARN;
            case E_PARSE:              return Core_BP_Raven_Client::ERROR;
            case E_NOTICE:             return Core_BP_Raven_Client::INFO;
            case E_CORE_ERROR:         return Core_BP_Raven_Client::ERROR;
            case E_CORE_WARNING:       return Core_BP_Raven_Client::WARN;
            case E_COMPILE_ERROR:      return Core_BP_Raven_Client::ERROR;
            case E_COMPILE_WARNING:    return Core_BP_Raven_Client::WARN;
            case E_USER_ERROR:         return Core_BP_Raven_Client::ERROR;
            case E_USER_WARNING:       return Core_BP_Raven_Client::WARN;
            case E_USER_NOTICE:        return Core_BP_Raven_Client::INFO;
            case E_STRICT:             return Core_BP_Raven_Client::INFO;
            case E_RECOVERABLE_ERROR:  return Core_BP_Raven_Client::ERROR;
        }
        if (PHP_VERSION_ID >= 50300) {
            switch ($severity) {
            case E_DEPRECATED:         return Core_BP_Raven_Client::WARN;
            case E_USER_DEPRECATED:    return Core_BP_Raven_Client::WARN;
          }
        }
    ...
    private function triggerAutoload()
    {
        ...
        if (! class_exists('Core_BP_Raven_Stacktrace')) {
            spl_autoload_call('Core_BP_Raven_Stacktrace');
        }
