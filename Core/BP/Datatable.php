...
class Core_BP_Datatable {
	...
	private $roHRBallbackFunctions=array();
	...
	function addFunctionRoHRBallback($jScbFunction){
		$this->roHRBallbackFunctions[]=$jScbFunction;
	}	
	...
	function draw() {
		...
		foreach($this->fields as $fname => $fattr) {
			if($fattr["permission_test_hide"]) {
				if(!Plugin_ACL::$fattr["permission_test_hide"](Core_BP_Session::getVal("user_id"))) {
					continue;
				}
			}
...
EOT;
	...
	function roHRBount($dont_wh=0,$maintablealias="") {
		if($this->countquery){
		...
		*/
		...
		if(count($this->arrUnionQueries)>0) {
			...
			try {
				...
				echo "sdg ".$sql."<br>".Core_BP_Exception::processException($e);
				exit;
			}
		} else {
			$_aux = explode(" ",$sql);
			...
			try {
				...
				echo $sql."<br>".Core_BP_Exception::processException($e);
				exit;
			}
		}
	}
...
//		$iTotalRecords = self::roHRBount(1, $main_table_alias);
	...
	function formatField($fname, $fattr, $row) {
		...
		$no_link=0;
		if($fattr["link_if"]) {
			//echo "return ".Core_BP_Tags::parse($fattr["link_if"],$row).";\n";
			$no_link = !eval("return (".Core_BP_Tags::parse($fattr["link_if"],$row).");");
		}
		if($fattr["link_permission"]) {
			$no_link = !call_user_method($fattr["link_permission"],new Plugin_ACL,Core_BP_Session::getVal("user_id"));
		}
		...
		if($fattr["link"] && !$no_link && $row[$fname]) {
			$l = Core_BP_Tags::parse($fattr["link"],$row);
			...
			foreach($vals as $v) {
				$target="";
				$l = Core_BP_Tags::parse($fattr["href"],$row, $fname, $v);
				if($fattr["target"]) {
			...
			if(is_array($this->arrSubQueries[$fname])) { 
				...
				if(is_array($this->arrSubQueries[$fname][1])) {
					...
					if($fattr["permission_test_userparam"]) {
						$_params[] = Plugin_ACL::$fattr["permission_test_userparam"](Core_BP_Session::getVal("user_id"));
					}
					try {
