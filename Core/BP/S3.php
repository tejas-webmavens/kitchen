...
class Core_BP_S3
{
	...
	* @return void
	*/
	public function generatePath($sub_path){
		$s3 = Core_BP_Helpers::coreRegistry('s3');
		$pathPrefix = $s3->pathPrefix;

		return $pathPrefix.$sub_path;
	}
	...
	*/
	public static function setTimeCorrectionOffset($offset = 0)
	{
		if ($offset == 0)
		{
			$rest = new Core_BP_Core_BP_S3Request('HEAD');
	...
	*/
	private static function __triggerError($message, $file, $line, $code = 0)
	{
		if (self::$useExceptions)
			throw new Core_BP_S3Exception($message, $file, $line, $code);
		else
	...
	*/
	...
	{
		$rest = new Core_BP_S3Request('GET', '', '', self::$endpoint);
		$rest = $rest->getResponse();
	...
	*/
	...
	{
		$rest = new Core_BP_S3Request('GET', $bucket, '', self::$endpoint);
	...
	*/
	...
	{
		$rest = new Core_BP_S3Request('PUT', $bucket, '', self::$endpoint);
	...
	*/
	...
	{
		$rest = new Core_BP_S3Request('DELETE', $bucket, '', self::$endpoint);
		$rest = $rest->getResponse();
	...
	*/
	public static function putObject($input, $bucket, $uri, $acl = self::ACL_PRIVATE, $metaHeaders = array(), $requestHeaders = array(), $storageClass = self::STORAGE_CLASS_STANDARD, $serverSideEncryption = self::SSE_NONE)
	{
		...
		$rest = new Core_BP_S3Request('PUT', $bucket, $uri, self::$endpoint);
	...
	*/
	...
	{
		$rest = new Core_BP_S3Request('GET', $bucket, $uri, self::$endpoint);
		if ($saveTo !== false)
		{
	...
	*/
	...
	{
		$rest = new Core_BP_S3Request('HEAD', $bucket, $uri, self::$endpoint);
		$rest = $rest->getResponse();
	...
	*/
	...
	{
		$rest = new Core_BP_S3Request('DELETE', $bucket, $uri, self::$endpoint);
		$rest = $rest->getResponse();
		if ($rest->error === false && $rest->code !== 204)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false)
	...
	*/
	public static function createDistribution($bucket, $enabled = true, $cnames = array(), $comment = null, $defaultRootObject = null, $originAccessIdentity = null, $trustedSigners = array())
	{
		...
		$rest = new Core_BP_S3Request('POST', '', '2010-11-01/distribution', 'cloudfront.amazonaws.com');
	...
	*/
	public static function getDistribution($distributionId)
	{
		...
		self::$useSSL = true; // CloudFront requires SSL
		$rest = new Core_BP_S3Request('GET', '', '2010-11-01/distribution/'.$distributionId, 'cloudfront.amazonaws.com');
		$rest = self::__getCloudFrontResponse($rest);
	...
	*/
	public static function updateDistribution($dist)
	{
		...
		self::$useSSL = true; // CloudFront requires SSL
		$rest = new Core_BP_S3Request('PUT', '', '2010-11-01/distribution/'.$dist['id'].'/config', 'cloudfront.amazonaws.com');
		$rest->data = self::__getCloudFrontDistributionConfigXML(
			$dist['origin'],
			$dist['enabled'],
			$dist['comment'],
	...
	*/
	public static function deleteDistribution($dist)
	{
		...
		self::$useSSL = true; // CloudFront requires SSL
		$rest = new Core_BP_S3Request('DELETE', '', '2008-06-30/distribution/'.$dist['id'], 'cloudfront.amazonaws.com');
		$rest->setHeader('If-Match', $dist['hash']);
		$rest = self::__getCloudFrontResponse($rest);
	...
	*/
	public static function listDistributions()
	{
		...
		$useSSL = self::$useSSL;
		self::$useSSL = true; // CloudFront requires SSL
		$rest = new Core_BP_S3Request('GET', '', '2010-11-01/distribution', 'cloudfront.amazonaws.com');
		$rest = self::__getCloudFrontResponse($rest);
		self::$useSSL = $useSSL;
	...
	*/
	public static function listOriginAccessIdentities()
	{
		...
		self::$useSSL = true; // CloudFront requires SSL
		$rest = new Core_BP_S3Request('GET', '', '2010-11-01/origin-access-identity/cloudfront', 'cloudfront.amazonaws.com');
		$rest = self::__getCloudFrontResponse($rest);
		$useSSL = self::$useSSL;
	...
	*/
	public static function invalidateDistribution($distributionId, $paths)
	{
		...
		$useSSL = self::$useSSL;
		self::$useSSL = true; // CloudFront requires SSL
		$rest = new Core_BP_S3Request('POST', '', '2010-08-01/distribution/'.$distributionId.'/invalidation', 'cloudfront.amazonaws.com');
	...
	*/
	public static function getDistributionInvalidationList($distributionId)
	{
		...
		$useSSL = self::$useSSL;
		self::$useSSL = true; // CloudFront requires SSL
		$rest = new Core_BP_S3Request('GET', '', '2010-11-01/distribution/'.$distributionId.'/invalidation', 'cloudfront.amazonaws.com');
		$rest = self::__getCloudFrontResponse($rest);
		self::$useSSL = $useSSL;
	...
	*
	* @internal Used to parse the CloudFront Core_BP_S3Request::getResponse() output
	* @param object &$rest Core_BP_S3Request instance
	* @return object
	*/
	...
	*
	* @internal Used by Core_BP_S3Request::getResponse()
