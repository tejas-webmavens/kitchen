...
class Core_BP_Date {
	...
	function count_days($start_time, $end_time){
		$date_today = new DateTime($start_time); 
        $date_due = new DateTime($end_time); 
        $interval = $date_due->diff($date_today); 
 
        return $interval->format('%a');
	}
	function count_hrs($start_time, $end_time){
		$diff = date_diff($start_time, $end_time);
		return $diff->h;
	}
	function count_min($start_time, $end_time){
		$diff = date_diff($start_time, $end_time);
		return $diff->i;
	}
	function count_sec($start_time, $end_time){
		$diff = date_diff($start_time, $end_time);
		return $diff->s;
	}
}
