<?php
class Core_BP_PHPMailer{
	var $from="";
	var $from_email="";
	private $log=0;
	var $standardauth=false;
	
	function __construct($from, $from_email) {
		$this->from = $from;
	...
	function send($to,$to_name,$cc,$bcc,$subject,$body,$attachments=array(),$from="") {
		...
		$smtp_config = Core_BP_Helpers::coreRegistry("smtp");
      ...
      if($attachments!=""){
		...
		if(count($attachments)) {
			foreach($attachments as $a) {
				if(is_array($a)) {
					$mail->AddAttachment($a[0],$a[1]);
				} else {
					//$mail->AddAttachment($a);
          $file_url_part = explode("/", $a);
          $file_name = $file_url_part[count($file_url_part)-1];

          $mail->AddStringAttachment(file_get_contents($a), $file_name);
				}
			}
		}
		$mail->SetMessageType();
	...
	public function getBody($view,$params){
		...
		$template = $db->fetchOne("SELECT template FROM email_templates WHERE view='$view'");
		if($template!=""){
...
class PHPMailer {
  ...
  function SmtpConnect() {
    ...
    while($index < count($hosts) && $connection == false) {
      $hostinfo = array();
      if(preg_replace('^(.+):([0-9]+)$', $hosts[$index], $hostinfo)) {
        $host = $hostinfo[1];
        $port = $hostinfo[2];
      } else {
        $host = $hosts[$index];
